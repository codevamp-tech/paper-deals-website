...
export default function BuyerList() {
    ...
    const fetchBuyers = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/users/getBuyer?page=${currentPage}&limit=10`
        );
  ...
  const categories = useMemo(() => {
    const allCats = buyers
      .flatMap(
        (buyers) => buyers.organization?.materials_used_names || []
      )
      .filter(Boolean);
  ...
  const filteredBuyers = buyers.filter((buyers) => {
    const materials = buyers.organization?.materials_used_names || [];
    return (
  ...
  const selectedBuyers = filteredBuyers.slice(
    startIndex,
  ...
  useEffect(() => {
    const fetchRatings = async () => {
      try {
        ...
        await Promise.all(
          selectedBuyers.map(async (buyers) => {
            try {
              const res = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL}/api/RatingReview/get?buyers_id=${buyers.id}`
              );

              const data = await res.json();

              if (data.success && data.data) {
                newRatings[buyers.id] = {
                  average: data.data.average_rating || 0,
                  reviews: data.data.review_count || 0,
                };
              } else {
                newRatings[buyers.id] = { average: 0, reviews: 0 };
              }
            } catch (error) {
              console.error(
                `Error fetching rating for buyers ${buyers.id}:`,
                error
              );
              newRatings[buyers.id] = { average: 0, reviews: 0 };
            }
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {selectedBuyers.map((buyers) => {
              const org = buyers.organization || {};
              const ratingData = ratingsData[buyers.id] || {
                average: 0,
                reviews: 0,
              };
