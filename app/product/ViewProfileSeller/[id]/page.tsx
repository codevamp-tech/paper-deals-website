...
export default function BuyersPage() {
  ...
  useEffect(() => {
    ...
    const fetchSeller = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/users/sellers/${id}`)
  ...
  // Normalize optional fields used in the design
  const typeOfSeller = org?.type_of_seller || "Seller"
  const capacity = org?.production_capacity_tpm || org?.production_capacity || ""
  const dealsIn = Array.isArray(org?.deals_in) ? org.deals_in.join(", ") : org?.deals_in || ""
    ...
    <main className="min-h-screen bg-background">
      <section className="max-w-5xl mx-auto px-4 py-8 md:py-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-6 md:gap-8">
                ...
                <div className="aspect-[4/3] w-full rounded-lg border bg-muted/30 overflow-hidden flex items-center justify-center">
                  {org?.image_banner ? (
                    ...
                    <div className="text-center">
                      ...
                      <p className="mt-2 text-xs text-muted-foreground">No company logo</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-col justify-center">
                <div className="flex flex-wrap items-center gap-3">
                  <h1 className="text-2xl md:text-3xl font-bold text-balance">{orgName}</h1>
                  <Badge
                    className={
                      isVerified
        ...
        <div className="mt-8 flex justify-center">
          {email ? (
            <a href={`mailto:${email}`} className="inline-flex" aria-label="Enquiry Now via Email">
